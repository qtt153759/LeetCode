...
def fun(i, mask):
    ...
    print("(", binaryToDecimal(i), ",", binaryToDecimal(mask), "): ", binaryToDecimal((1 << i) | 3))
    if mask == ((1 << i) | 3):
        print("start", binaryToDecimal((1 << i | 3)), binaryToDecimal(mask))
        return dist[1][i]
    ...
    for j in range(1, n + 1):
        print(
            ...
            i,
            ",",
            binaryToDecimal(mask),
            ",",
            binaryToDecimal((1 << j)),
            "): ",
            binaryToDecimal((mask & (1 << j))),
        )
        if (mask & (1 << j)) != 0 and j != i and j != 1:
            print("co ", binaryToDecimal(mask & (~(1 << i))), " va ", binaryToDecimal((~(1 << i))))
