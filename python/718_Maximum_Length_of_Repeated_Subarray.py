...
class Solution:  # 156 ms, faster than 98.97%
    def findLength(self, nums1: List[int], nums2: List[int]) -> int:
        ...
        for i in range(max(m, n)):
            POW[i + 1] = POW[i] * BASE % MOD  # Compute POW of BASE
        for i in range(m):
            hash1[i + 1] = (hash1[i] * BASE + nums1[i]) % MOD  # Compute hashing values of nums1
        for i in range(n):
            hash2[i + 1] = (hash2[i] * BASE + nums2[i]) % MOD  # Compute hashing values of nums2
        ...
        def foundSubArray(size):
            ...
            for i in range(n - size + 1):
                ...
                if h in seen:
                    for j in seen[
                        h
                    ]:  # Double check - This rarely happens when collision occurs -> No change in time complexity
